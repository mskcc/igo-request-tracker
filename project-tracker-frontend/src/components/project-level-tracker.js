    ...
    const getOtherTimeField = () => {
        if (recentDeliveryDate && 'Not Available' !== recentDeliveryDate) {
            return (
                <Col xs={12} sm={6}>
                    <p className={'text-align-left'}>
                        <span className={'bold'}>Recent Delivery Date</span>: {recentDeliveryDate}
                    </p>
                </Col>
            );
            ...
            return (
                <Col xs={12} sm={6}>
                    <p className={'text-align-left'}>
                        <span className={'bold'}>Turn Around Time From In Processing</span>: {tatFromInProcessing}
                    </p>
                </Col>
            );
        }
        return (
            <Col xs={12} sm={6}>
                <p className={'text-align-left'}>
                    <span className={'bold'}>Turn Around Time From Receiving</span>: {tatFromReceiving}
                </p>
            </Col>
        );
    };
    ...
    return (
        <Container className={'border'}>
            <Row className={'padding-vert-10 border'}>
                <Col xs={12} sm={6}>
                    <p className={'text-align-left'}>
                        <span className={'bold'}>Project Start</span>: {startTime}
                    </p>
                </Col>
                {getOtherTimeField()}
                <Col xs={12} sm={6}>
                    <p className={'text-align-left'}>
                        <span className={'bold'}>Group Leader</span>: {groupLeader}
                    </p>
                </Col>
                <Col xs={12} sm={6}>
                    <p className={'text-align-left'}>
                        <span className={'bold'}>Investigator</span>: {investigator}
                    </p>
                </Col>
                <Col xs={12} sm={6}>
                    <p className={'text-align-left'}>
                        <span className={'bold'}>Lab Head Email</span>: {labHeadEmail}
                    </p>
                </Col>
                <Col xs={12} sm={6}>
                    <p className={'text-align-left'}>
                        <span className={'bold'}>Laboratory Head</span>: {labHead}
                    </p>
                </Col>
            </Row>
            <Row className={'parent-sample-level-stages'}>
                <StageLevelTracker stages={stages} orientation={'horizontal'} projectView={true}></StageLevelTracker>
            </Row>
                            ...
                            <Row>
                                {completedSamples.length > 0 ? (
                                    <Col xs={6} sm={4}>
                                        Completed
                                        <span
                                            className={`small-icon mskcc-black fa-layers fa-fw hover inline-block ${
                                                showCompleted ? '' : 'fade-color'
                                            }`}
                                ...
                                {failedSamples.length > 0 ? (
                                    <Col xs={6} sm={4}>
                                        Failed
                                        <span
                                            className={`small-icon mskcc-red fa-layers fa-fw hover inline-block ${
                                                showFailed ? '' : 'fade-color'
                                            }`}
                                ...
                                {pendingSamples.length > 0 ? (
                                    <Col xs={6} sm={4}>
                                        Pending
                                        <span
                                            className={`small-icon mskcc-dark-blue fa-layers fa-fw hover inline-block ${
                                                showPending ? '' : 'fade-color'
                                            }`}
