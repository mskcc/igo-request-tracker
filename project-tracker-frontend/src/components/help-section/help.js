...
function HelpSection() {
    ...
    return (
        <div className={'border padding-vert-10 padding-hor-20'}>
            <div className={'padding-hor-20 padding-vert-10'}>
                ...
                <p>
                    {' '}
                    Below is brief documentation of the request tracker and how it can be used. Feedback is always welcome, if something
                    doesn't make sense or you have questions, please submit a request on Teamworks{' '}
                    <span>
                        <FontAwesomeIcon onClick={goToTeamWorks} className={'hover'} icon={faUsers} />
                    </span>
                    .
                </p>
            </div>
            <div className={'padding-hor-20 padding-vert-10 border'}>
                <div onClick={() => toggleShow(REQUEST_SECTION)} className={'hover'}>
                    ...
                    <FontAwesomeIcon
                        className="request-selector-icon inline-block float-right"
                ...
                {showRequestSections ? (
                    <div className={'margin-left-20'}>
                        <p>
                            There are two sections for for {getHumanReadable(STATE_PENDING_REQUESTS)} and{' '}
                            {getHumanReadable(STATE_DELIVERED_REQUESTS)}
                        </p>
            ...
            <div className={'padding-hor-20 padding-vert-10 border'}>
                ...
                {showRequestView ? (
                    <div className={'margin-left-20'}>
                        <p>
                            In each section, requests available to the logged-in user will begin populating. Loading requests will have an
                            ellipsis icon.
                        </p>
                        <img className={'help-img'} src={requestViewMultiple} alt={'request-view-multiple'}></img>
                        <p>
                            Once loaded, requests will display an icon on the right that indicates the stage completion status of the
                            request samples in the workflow. For more information on stages, please take a look at the "Stages" section
                        </p>
                        ...
                        <p>
                            To investigate individual samples in the request, toggling the "View Samples" button will display more samples
                        </p>
            ...
            <div className={'padding-hor-20 padding-vert-10 border'}>
                ...
                {showStageSection ? (
                    <div className={'margin-left-20'}>
                        <p>
                            {' '}
                            Stages are the steps an IGO sample must progress through before being delivered. Workflows are categorized into
                            the stage they belong to. For example, the "10x Genomics Library Preparation" workflow belongs to the "Library
                            Prepration" stage. A sample or request with a pending stage is waiting for a sample in that stage to progress.
                        </p>
                        <p>
                            {' '}
                            A request will have the stages of the samples in that request. However, it is important to note that a request
                            will be more conservative than one of its samples in reporting its overall progress.{' '}
                            <span className={'bold'}>
                                A request will report it has completed a stage ONLY if all samples in that request have completed that stage
                            </span>
                            . This means that if there is a lagging sample, the request will report it hasn't completed a stage even if most
                            of the samples are at a more advanced stage in the workflow. For instance, below one sample is still pending
                            "Library Preparation" even though two samples have advanced to "STR Analysis". The request will remain in the
                            "Library Preparation" stage until the lagging sample has either failed or completed that stage.
                        </p>
                        ...
                        <ul>
                            A request is considered to have completed a stage if all of its samples have completed that stage. For example,
                            a request with four samples at stage II and two samples at stage III will report that it is pending stage II.
                        </ul>
                        <h2>How are stages determined?</h2>
                        <p>
                            {' '}
                            Every sample IGO receives enters a workflow defined in our Laboratory Information Management System (LIMS). It
                            is tracked in our LIMS through each workflow by creating representative data records at each step in a workflow.
                            Each one of these data records has a status that summarizes the step the physical sample was at. This status
                            record of a data record is mapped to a corresponding stage (below).
                        </p>
                                ...
                                <Table aria-label="stage-mapper">
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            <TableCell>
                                                <span className={'bold'}>Workflow</span>
                                            </TableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                        ...
                        <h3>Special Cases</h3>
                        <p>
                            <span className={'bold'}>Data QC</span>
                        </p>
                        <p>
                            {' '}
                            The Data QC stage is determined by whether a LIMS Sample Data Record is "linked" to a "SeqAnalysisSampleQC" Data
                            Record. This is unlike the other stages for which there is a direct mapping of status-to-stage, the Data QC
                            stage.
                        </p>
                    </div>
                ) : (
