...
function SampleTree({ sample, idx }) {
    ...
    return (
        <Row key={sampleId} className={'border'}>
            <Col xs={3} sm={2} md={1} className={'padding-vert-10'}>
                <span className={toggleClasses} onClick={() => setShowTree(!showTree)}>
            ...
            {showTree ? (
                <Col xs={12} className={'sample-tree-container'} style={treeContainerStyle}>
                    ...
                    <Tree data={sample.root} translate={translate} orientation={'horizontal'} nodeSize={{ x: 300, y: 100 }} zoom={0.5} />
                </Col>
            ) : (
                <div></div>
            )}
        ...
        <Container>
            {samples.length > 0 ? (
                samples.map((sample, idx) => {
                    return <SampleTree sample={sample} idx={idx} key={`${sample}-${idx}`}></SampleTree>;
                })
            ) : (
                <Row>
                    <Col xs={12}>
