...
function ProjectTracker({ projectName, projectState }) {
    ...
    useEffect(() => {
        if (!projectHasData(project)) {
            ...
            getProjectTrackingDataRequest(projectName).then((data) => {
    ...
    const getSummaryIcon = (projectName) => {
        ...
        if (mapping === null || mapping === undefined) {
            return (
                <span className={`float-right small-icon width-100 mskcc-black fa-layers fa-fw hover inline-block`}>
        ...
        if (isIgoComplete) {
            return (
                <span className={`float-right small-icon fa-layers fa-fw hover inline-block width-100 mskcc-dark-green`}>
        ...
        return (
            <span className={`float-right large-icon fa-layers fa-fw hover inline-block width-100 ${summaryColor}`}>
                <FontAwesomeIcon icon={faFlask} />
                <span className="fa-layers-bottom fa-layers-text fa-inverse project-summary-text-override">
                    {completed}/{total}
                </span>
            </span>
        );
    ...
    return (
        <div>
            <div className={'hover border padding-vert-5 padding-hor-20'} onClick={() => setShowProject(!showProject)}>
                ...
                {projectHasData(project) && project.getIgoComplete() ? (
                    <FontAwesomeIcon className="request-complete" icon={faCheck} />
                ) : (
                    <span></span>
                )}
