...
const Feedback = ({ closeFeedback }) => {
    ...
    return (
        <div className={'feedback-form padding-24'}>
            <FontAwesomeIcon className={'status-change-close hover'} icon={faTimes} onClick={closeFeedback} />
            <div className={'feedback-container margin-hor-20'}>
                <form className={'fill-width'}>
                    <h5 className={'text-align-center bold'}>Feedback?</h5>
                    <p className={'text-align-left'}>Please fill out a teamworks request</p>
                    <div className={'fill-width text-align-center'}>
                        <IconButton aria-label="teamworks-link" onClick={goToTeamWorks} className={'border hover'}>
                            <FontAwesomeIcon className={'hover'} icon={faUsers} />
                        </IconButton>
                    </div>
                </form>
            </div>
