        ...
        <div className={'border'}>
            <Container>
                <Row className={'black-border backgorund-light-gray padding-vert-20 padding-hor-20'}>
                    ...
                    <Col xs={4}>
                        <h4>
                            Total {getHumanReadable(projectState)}: {Object.keys(projectMapping).length}
                        </h4>
                    </Col>
                    <Col xs={6}>{generateTextInput('Request ID', query, setQuery)}</Col>
                </Row>
            </Container>

            {filtered.map((projectName) => {
                return <ProjectTracker key={projectName} projectName={projectName} projectState={projectState} />;
            })}
        </div>
    );
}
